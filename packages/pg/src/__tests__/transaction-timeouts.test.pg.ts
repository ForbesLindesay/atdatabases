...
test(`lock timeout can be set overall and per transaction`, async () => {
  ...
  );
  {
    ...
    const end = Date.now();
    expect(Math.abs(500 - (end - start))).toBeLessThan(100);
  }
  {
    const start = Date.now();
    await expect(async () => {
    ...
    const end = Date.now();
    expect(Math.abs(1000 - (end - start))).toBeLessThan(100);
  }
  {
    const start = Date.now();
    await expect(async () => {
    ...
    const end = Date.now();
    expect(Math.abs(2_000 - (end - start))).toBeLessThan(100);
  }
  {
    const start = Date.now();
    await expect(async () => {
    ...
    const end = Date.now();
    expect(Math.abs(3_000 - (end - start))).toBeLessThan(100);
  }
...
test(`idle in transaction timeout can be set overall`, async () => {
  ...
  expect(Math.abs(1_000 - (await timeUntilLockAvailable()))).toBeLessThan(100);
  releaseLock.resolve(undefined);
...
test(`idle in transaction timeout can be per transaction`, async () => {
  ...
  expect(Math.abs(500 - (await timeUntilLockAvailable()))).toBeLessThan(50);
  releaseLock.resolve(undefined);
  await expect(lock).rejects.toMatchInlineSnapshot(
    `[error: terminating connection due to idle-in-transaction timeout]`,
  );
